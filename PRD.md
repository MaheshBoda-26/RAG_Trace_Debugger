...
- **Time**: 9-day build window (Round 2: July 15 night – July 24), plus a final polish day (July 25) before live presentation. No time budget for production hardening, auth, or multi-tenancy.
- **Team size**: assume a single builder or a very small team (1–2 people) given the hackathon format; scope is deliberately kept to what one AI engineer can build and demo solo.
- **Budget**: no paid infrastructure assumed. Local/free-tier tooling only (SQLite over managed Postgres, open-source embedding/rerank models where possible, a single LLM API budget kept small since the demo corpus is intentionally small).
- **Environment**: must run and demo live at OJONE, Bengaluru — needs to work reliably on a laptop without dependency on unstable network calls where avoidable.

## 10. Assumptions

- The demo RAG pipeline is built from scratch for this project rather than integrated into OneInbox's actual production system; the debugger's value is demonstrated on a controlled example, not validated against OneInbox's real traffic.
- Judges will evaluate based on the live demo and evaluation results, not just the written documents, so the working dashboard takes priority over polish elsewhere.
- A small (10–20 document) test corpus is sufficient to demonstrate the concept; no claim is made that this generalizes to enterprise-scale document volumes without further work.
- One LLM provider (e.g., an existing API key/account) is available for the generation stage; no dependency on OneInbox-specific infrastructure or credentials.
