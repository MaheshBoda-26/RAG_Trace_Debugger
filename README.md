...
<!-- EMBED VIDEO UPGRADE: GitHub's inline video player only works with github.com/user-attachments/assets/... URLs, which are generated by uploading the mp4 through GitHub's web UI (drag the file into a new issue's comment box, submit, copy the resulting URL, then close the issue without posting). Replace the <a> wrapper around <img src="docs/demo-poster.jpg"> below with [![Watch the demo](docs/demo-poster.jpg)](PASTE_USER_ATTACHMENTS_URL_HERE) — or, once you have that URL, replace this whole block with the URL on its own line (a bare user-attachments URL renders as an inline playable player). This repo-relative fallback works everywhere and clicks through to the video. -->

### 🎬 Watch it work (17s)

[![RAG Trace Debugger demo: a wrong answer is traced stage-by-stage, the failing rerank stage is stamped as the verdict, and a one-click re-test flips the result to improved](docs/demo-poster.jpg)](docs/demo.mp4)

*Click the poster to watch. In 17 seconds: a confidently wrong answer gets autopsied stage by stage — retrieval was fine, **rerank dropped the one chunk that had the answer** — the verdict is stamped on the failing stage, and one click re-runs the case with fixed parameters and flips the result to improved.*
...
│   └── demo.mp4         # 17s product video (poster: demo-poster.jpg), embedded at the top of this README
